...
def obtener_producto(producto_id: int, session: Session = Depends(get_session)):
    ...
    producto = service.get_by_id(session, producto_id)
    if not producto:
        raise HTTPException(
...
def actualizar_producto(producto_id: int, data: ProductoUpdate, session: Session = Depends(get_session)):
    ...
    producto = service.update(session, producto_id, data)
    if not producto:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def eliminar_producto(producto_id: int, session: Session = Depends(get_session)):
    ...
    eliminado = service.delete(session, producto_id)
    if not eliminado:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Producto con id {producto_id} no encontrado"
...
def listar_ingrediente_relaciones(session: Session = Depends(get_session)):
    ...
    return service.get_all_ingrediente_relaciones(session)
...
def crear_ingrediente_relacion(data: ProductoIngredienteCreate, session: Session = Depends(get_session)):
    ...
    relacion = service.create_ingrediente_relacion(session, data)
    if not relacion:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def eliminar_ingrediente_relacion(producto_id: int, ingrediente_id: int, session: Session = Depends(get_session)):
    ...
    eliminada = service.delete_ingrediente_relacion(session, producto_id, ingrediente_id)
    if not eliminada:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
